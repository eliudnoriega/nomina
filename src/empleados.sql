...
CREATE TABLE `t_empleado` (
  ...
  `PUESTO` varchar(5) COLLATE utf8_spanish_ci DEFAULT NULL,			-- TIPO
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_empleado` (IN `SDNI_EMP` CHAR(8), IN `SID_DEPARTAMENTO` VARCHAR(50), IN `SSALARIO` CHAR(8), IN `SPUESTO` VARCHAR(5), IN `SCUENTA_BANCARIA` VARCHAR(12), IN `SNOMBRE` VARCHAR(50), IN `SPRIMER_APE` VARCHAR(50), IN `SSEGUNDO_APE` VARCHAR(50))  NO SQL
BEGIN
